...
> **Last updated:** July 30, 2026 — S93 follow-up (#111 and #112 filed: instrument_rates UTC date cap and unserialized backdating floor, both documented-accepted)
...
- **#111** instrument_rates date cap uses UTC — DOCUMENTED-ACCEPTED (Josh, S93 follow-up). The `instrument_rates_backdating_guard` trigger (migration 20260730010000, unapplied) rejects `effective_from > CURRENT_DATE`, which is the DB's UTC date. A user in a timezone ahead of UTC entering "today" late in their day can be rejected as future-dated; a user behind UTC can date a rate one day "ahead" of their local today. `companies.timezone` (migration 20260719000000) exists if this ever needs a company-local date. Accepted as-is — revisit only if it bites a real user. Same class as the #92 read-time-window posture.

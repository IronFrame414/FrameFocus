...
- **#157** **`desktop-chat-switcher.spec.ts:62` flaked on CI #304 and is UNRESOLVED — it is either a real defect in the per-thread unread count or the fingerprint of two CI runs sharing one database. Filed so the next CLEAN run decides it, because nothing else will.** Observed S107b (2026-09-10), on the run that ended CI's red streak since #290.
  ...
  **The candidate mechanical fix for the collision arm, verified rather than suggested from memory:** `.github/workflows/ci.yml` has **no `concurrency:` key at all** — every match on that word in the file is prose in a comment. A workflow-level `concurrency` group with `cancel-in-progress` would have prevented #303/#304 overlapping in the first place, and it does not depend on the seed (#149) or on sharding. **It is a candidate, not a ruling** — it interacts directly with §14b's *proposed* push-hold exception, which is **unruled**, and cancelling a run has its own cost. Note also that the Codespace `GITHUB_TOKEN` lacks `actions:write` (`POST /actions/runs/{id}/cancel` → **403**), so an agent that starts a duplicate run cannot clean it up by hand — which is exactly why the fix has to be declarative if it happens at all.

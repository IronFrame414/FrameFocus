...
> **Last updated:** September 10, 2026 — S107b (**#157 RAISED** — `desktop-chat-switcher.spec.ts:62` flaked on CI #304, the run that ended the red streak since #290. **Unresolved on purpose:** it is either a real defect in the per-thread unread count or the fingerprint of runs #303 and #304 driving one `rebuild-test` at once, and the contaminated window makes the two indistinguishable from the log. **Filed as its own number rather than as an amendment to #150** — #150 is intra-run cross-SHARD and dormant since the S134 revert, this was an unsharded `workers: 1` run colliding with a second RUN; and #150 holds a prediction where this holds an observation. Decided by one solo run of the file on an idle DB. #150 amended with a pointer, not folded. **§14a AND §14b RULED [Josh], as ONE change in this same commit:** `ci.yml` now triggers on push to **every** branch and carries a `concurrency` group with `cancel-in-progress` — it previously had **no `concurrency:` key at all**, which is why #303 and #304 could collide. Separated, the two make things worse; together they are net fewer concurrent runs than before. **The §14b push-hold exception is dead and deliberately NOT written into the rule** — cancellation removes the conflict at source; recorded as made-unnecessary, not forgotten. **#304 stays green:** contention manufactures false reds, never false greens.)
> **Previously:** September 1, 2026 — S179 (**THE DEBT/OWED-WORK SPLIT, RULED [Josh].** `TECH_DEBT.md` is for decided-to-live-with or deliberately-deferred **decisions**, not a backlog. **#155 RAISED** — custom composable roles, converted from `#1-regbacklog`; **#156 RAISED** — the safety-incident fan-out, converted from `#1-email` (the prompt called it `#3-email`; the real ledger id was `#1-email`). Both are deferred decisions and stay debt. **Reclassified OUT to the register as owed work:** `#2-regbacklog` (A15 unbilled-to-client), `#3-regbacklog` (A16 package rename — the "~150" imports is really **340**), `#4-regbacklog` (K8 duplicate tokens, verified at `theme.ts:45-49`), and `#1-delsweep` (the DMARC `rua` — ✅ its "gmail repoint does not work" finding was **ruled correct [Josh]**; register §Q3's repoint-to-Gmail ruling **overturned**, and §O8 now carries the RFC 7489-verified TXT record to publish). Provisional entries superseded-in-place, never deleted.)
...
- **#150** **Four CI shards shared ONE rebuild-test database, so any test asserting the ABSENCE / emptiness / exact COUNT of something another shard writes to a shared fixture was exposed. Recorded precisely so a future sharding attempt starts from this list, not a fresh audit.** Raised S134 (2026-08-11). The sharding that caused it (S133, `ce6efa8`) is reverted for now (Option D, #149); the sharding work is kept on branch `ci/shard-playwright`, not deleted, for when the seed (#149) lands.
  ...
  **The exposed set — concentrated in two subsystems, not diffuse:**
  ...
  - **Chat on the three shared chat projects.** `desktop-chat-poll.spec.ts:86/91/96` assert **exact thread totals** (`toHaveCount(25/30/0)`); five other files (`desktop-chat-send`, `-mentions`, `-switcher`, `-sub`, `m-chat-sub`) both send to and **`teardownChat([project])`** the same threads. **Teardown is per-PROJECT**, so a teardown on one shard can delete another shard's in-flight chat data — the ~11-file chat suite is mutually fragile on shared threads, broader than the count assertions alone. `desktop-chat-switcher` unread badges are a borderline second case. **[S107b] That prediction has now FIRED ONCE in the wild — `desktop-chat-switcher.spec.ts:62` flaked on CI #304 — but under a DIFFERENT mechanism (two concurrent CI runs, not two shards) and it is unresolved. Filed separately as #157; do not fold it in here.**
...
- **#157** **`desktop-chat-switcher.spec.ts:62` flaked on CI #304 and is UNRESOLVED — it is either a real defect in the per-thread unread count or the fingerprint of two CI runs sharing one database. Filed so the next CLEAN run decides it, because nothing else will.** Observed S107b (2026-09-10), on the run that ended CI's red streak since #290.

  **The observation, exactly.** CI **#304** (`f853663`, `event: pull_request`) was **green** — 557 passed · 10 skipped · 0 failed, both jobs `completed/success`, `upload-artifact` skipped. Inside that green tally, one line:
  ...
  It **passed on retry**. It was not chased, deliberately, and the reason is recorded rather than assumed: #304's e2e job ran 00:59:18 → 01:27:03 while the duplicate run **#303 was not cancelled until 01:25:40**, so **almost the entire run overlapped another suite driving the same `rebuild-test`**. An unread-COUNT assertion is precisely the shape a foreign run's chat writes disturb. Diagnosing one retry-passing flake out of a knowingly contaminated window is guesswork dressed as analysis.

  **What the test asserts, and why it is exposed.** `desktop-chat-switcher.spec.ts:62-71` signs in as OWNER, opens the launcher, and asserts an **exact** unread count on the shared chat project `PROJECT_QA_A` (`4a4f8567-…`, `chat-fixture.ts:26`) — both `data-unread` **`'1'`** and the badge text **`'1'`**. **Eight other spec files import `PROJECT_QA_A` and eight call `teardownChat()`**, which is **per-PROJECT**: `desktop-chat-panel`, `-mentions`, `-poll`, `-send`, `-photos`, `-sub`, `m-chat-sub`, `m-chat-offline`. Within one serial run (`workers: 1`) those are ordered and safe. Across two concurrent runs they are not, and no configuration in this repo makes them so.

  **⚠️ WHY THIS IS ITS OWN NUMBER AND NOT AN AMENDMENT TO #150 — decided, not defaulted.** #150 already names *"`desktop-chat-switcher` unread badges … a borderline second case"*, and the temptation is to append this there. **Three reasons not to:**

  1. **The mechanism is different.** #150 is **intra-run, cross-SHARD** — four `--shard=k/n` slices of one run against one DB. That sharding is **reverted** (Option D, S134) and #150 is dormant by design: its own framing is *"recorded precisely so a future sharding attempt starts from this list."* This flake happened on an **unsharded, `workers: 1` serial run**. What collided was **run against run** — #303 against #304 — a concurrency `workers: 1` cannot see or control (§14b). Filing a live observation inside a dormant entry buries it until someone resumes sharding, which may be never.
  2. **The epistemic status is different, and that is the point of the entry.** #150's mention is a **prediction** made by reading the code — a case reasoned to be exposed. This is an **observation** with a run number, a timestamp window, a test id and a pass-on-retry. Folding the second into the first loses the only thing that makes it actionable: that it has now actually fired once, in the wild, and nobody knows which of two causes it was.
  3. **The next step differs.** #150's fix direction is #149's reproducible seed → DB-per-shard. **This needs one thing and it is nearly free:** a single **solo** run of `desktop-chat-switcher` with nothing else against `rebuild-test`.

  **The decision procedure, so it is not re-derived.** Run the file alone, on an otherwise-idle `rebuild-test`, several times.
  - **If it recurs on a solo run it is REAL** and belongs to `desktop-chat-switcher` / the per-thread unread count — a product or test defect, and this entry becomes its diagnosis.
  - **If it does not recur, it was the collision** — this entry closes as *not-a-defect* and its evidence goes to strengthen §14b's case, at which point #150's "borderline second case" is upgraded from prediction to **confirmed**, cross-referenced here.

  **⚠️ RULED [Josh, S107b] — THE COLLISION ARM IS FIXED IN CONFIG, AND §14a AND §14b ARE ONE CHANGE.** `.github/workflows/ci.yml` now triggers on push to **every** branch and carries a **`concurrency` group with `cancel-in-progress: true`**.

  _Superseded text, quoted rather than rewritten:_ _"**It is a candidate, not a ruling** — it interacts directly with §14b's proposed push-hold exception, which is **unruled**, and cancelling a run has its own cost."_ It is now a ruling. The verified finding it rested on stands unchanged: there was **no `concurrency:` key in that file at all**, and every match on the word was prose in a comment.

  **§14a — trigger on any branch.** Josh's reason, recorded because it reframes the campaign's whole red streak: **every red in this campaign appeared AFTER the merge that caused it, and that is not a discipline failure — it is what the trigger config allowed.** Pushes and merges happen in one sequence, so a signal scoped to `main` always arrives behind the merge it was meant to gate. Branch triggers put the red in **front** of the merge, and cost no extra discipline, because the unattended-run rule already requires a push after every commit. The pushes were always happening; nothing was watching them.

  **§14b — cancel the superseded run.** No `concurrency` key is exactly why #303 and #304 could coexist on one `rebuild-test`. With `cancel-in-progress`, pushes stop piling up runs and the newest wins.

  **They are ONE change, not two, and separating them makes things worse:** branch triggers alone multiply runs and worsen the collision; the group alone leaves the signal arriving too late. Together they are **net fewer concurrent runs than the config they replace**.

  **Two implementation details that are load-bearing, recorded so a later edit does not undo them:**
  1. **The group key is `${{ github.head_ref || github.ref_name }}`, not `github.ref`.** Once a PR is open one push fires **both** a `push` and a `pull_request` event; grouping on `github.ref` puts them in **different** groups (`refs/heads/X` vs `refs/pull/N/merge`), so both run against one database — the #303/#304 collision reintroduced by the change meant to end it. `head_ref` (pull_request only) and `ref_name` (push) resolve to the same branch string, so the two events collapse into one group.
  2. **`'**'`, not `'*'`.** `'*'` does not match a branch name containing a slash, and every branch in this repo is `feature/…`, `fix/…` or `docs/…`.

  **✅ THE PUSH-HOLD EXCEPTION IS DEAD, AND DELIBERATELY NOT WRITTEN INTO THE RULE [Josh, S107b].** §14b proposed one — hold the push while a run is in progress, batch and send on a terminal state — because the unattended-push rule and a running CI job were in direct conflict. **`cancel-in-progress` dissolves that conflict at the source:** a superseded run cancels rather than competes, so there is nothing to hold for. Recorded as *proposed and made unnecessary*, not as forgotten, so nobody re-derives it. **CLAUDE.md's unattended push rule is unchanged and needs no carve-out.**

  **And the cancelling applies to `main` too** — considered, not overlooked. A run is only ever cancelled by a **newer** run of the same branch, so a most-recent run always exists; on `main` the state that matters is the tip, and the newer run tests a tree that *includes* the commit whose run was cancelled. The only thing lost is a result for an intermediate commit that no longer represents `main`.

  **What this does for #157 itself:** the clean solo run that decides this entry is now much easier to obtain, because runs on a branch no longer overlap each other. It does **not** decide it — a *second* consumer of `rebuild-test` (a local suite, another branch's run) still collides, since the group is per-branch and the database is not.

  **The asymmetry that keeps this small, and it is the one the whole S107 campaign turns on:** **contention manufactures false REDS, never false GREENS.** #304's green is trustworthy *despite* the overlap — nothing about interference makes a failing assertion pass. The flake is the only thing the overlap puts in doubt, and a flake is exactly what it would produce. **Nothing about this entry qualifies #304 as green or the merge at `1594fef` as sound.**

  **Numbering note.** Allocated as a bare `#157` on a branch, against CLAUDE.md → "Tech-debt numbering" (which requires a provisional `#N-<tag>` on a branch), **by Josh's explicit instruction for this filing**. The authority block at the top of this file is advanced to **#158** in the same commit, which is the step that actually prevents the collision that rule guards against. Cross-ref **#150**, **#149**; source `docs/sessions/S107-live-suite-repair.md` §14b and §15.

-- S111 Part Two — photo routing at conversion, and the conversion re-point
-- admitted by the site-visit freeze.
--
-- ⚠️ REBUILD-TEST FIRST. Production only when Josh applies it (Q20: nothing merges
-- before its migration is on production). No CHECK, no NOT NULL, no backfill: this
-- migration redefines two functions and governs no existing row.
--
-- ============================================================================
-- 1. convert_estimate_to_project — images become photos as they move [Q14 → D]
-- ============================================================================
-- Before: the files re-point (20261550000000:184-187) moved an estimate's files to
-- the new project but left category = 'other', so site-visit and estimate images
-- landed under Files, not Photos, and /m could not mark them up (getPhoto() keys on
-- category = 'photos'). The write path is fixed too (the estimate-files route now
-- writes 'photos' for images); this half catches everything ALREADY sitting on an
-- unconverted estimate, with no production data change.
--
-- The body below is the live function VERBATIM (md5 of prosrc on rebuild-test,
-- 2481c057f8b348a9306336e51510888a, equals 20261550000000's body) with ONE
-- statement changed: the files re-point.
--
-- ============================================================================
-- 2. enforce_site_visit_file_freeze — admit the conversion re-point
-- ============================================================================
-- Found at S111: the S110 A freeze (20261730000000) refuses ANY change to
-- estimate_id on a frozen site-visit capture, and frozen_at moves to now() when the
-- estimate is ACCEPTED. So after the normal flow — visit, send, accept — every
-- capture is frozen, and the conversion's re-point (estimate_id -> NULL) raises
-- "This site-visit photo is frozen", rolling the whole conversion back. The freeze
-- was written to stop a USER rewriting evidence; moving the evidence, untouched, to
-- the project the estimate became is not that.
--
-- The exemption is DERIVED FROM DATA, not from a session flag a caller could set:
-- the change must be estimate_id -> NULL with project_id set to the project whose
-- source_estimate_id is OLD.estimate_id (the row conversion just inserted), and
-- nothing else the freeze protects may change. Category may change (Q14) — it is
-- not in the freeze list, and never was.
...
BEGIN
  ...
  -- is complete.
  -- [S111 Part Two, RULED Q14 → D] An image still classified 'other' becomes a
  -- PHOTO as it moves, so it lands under Photos (where markup lives) instead of
  -- Files. Only 'other' — an image someone deliberately filed elsewhere keeps its
  -- category. Guarded on the company having the 'photos' key (files.category is an
  -- FK to file_categories); a company without it keeps 'other' rather than
  -- aborting the conversion.
  -- The freeze trigger (enforce_site_visit_file_freeze) admits this re-point for
  -- frozen captures — see the second half of this migration.
  UPDATE files
     SET project_id = v_project_id,
         estimate_id = NULL,
         category = CASE
           WHEN files.mime_type LIKE 'image/%'
            AND files.category = 'other'
            AND EXISTS (SELECT 1 FROM file_categories fc
                         WHERE fc.company_id = files.company_id AND fc.key = 'photos')
           THEN 'photos'
           ELSE files.category
         END
   WHERE estimate_id = v_estimate.id;
...
BEGIN
  ...
  IF v_frozen IS NULL OR OLD.created_at > v_frozen THEN
    RETURN NEW;
  END IF;
  -- [S111] The conversion re-point: estimate -> the project it became. See header.
  v_repoint := OLD.estimate_id IS NOT NULL
    AND NEW.estimate_id IS NULL
    AND NEW.project_id IS NOT NULL
    AND EXISTS (SELECT 1 FROM projects p
                 WHERE p.id = NEW.project_id AND p.source_estimate_id = OLD.estimate_id);
  IF NEW.is_deleted IS DISTINCT FROM OLD.is_deleted
     ...
     OR (NEW.estimate_id IS DISTINCT FROM OLD.estimate_id AND NOT v_repoint)

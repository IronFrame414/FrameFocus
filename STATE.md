...
| Supabase CLI            | ✅ Installed     | **Link to `nmyphyhmfttxkdoposvf` (rebuild-test) — NOT production.** Re-link after Codespace rebuild; verify with `npx supabase projects list` (● must be rebuild-test) before every push. Production (`jwkcknyuyvcwcdeskrmz`) is linked only for a supervised release. _(Corrected S122 — this row said "Linked to jwkcknyuyvcwcdeskrmz", matching the stale instruction in `scripts/session-start.sh`.)_ |
...
> | --------------------------- | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
> | **CO writes**               | foreman, crew, sub | **THE DATABASE.** `change_orders_{insert,update}_authorized` + both child tables. The route guard makes the refusal _honest_; it is not the refusal.               |
...
# ⚠️ [S107] A production sb_secret_ key was found in an account-level Codespaces
# secret named SUPABASE_SECRET_KEY, granted to this repo. Revoked at Supabase and
# the secret deleted. Nothing in the repo read that name — it was one rename from
# live on Josh's real books. Do not document production credentials in this file.
...
>
> **⚠️ EMAIL CONFIRMATION MUST STAY ENABLED.** S160 ruled that _invited_ users skip confirmation,
...
>   vendor row _QA Subcontractor Co (TEST IDENTITY)_, and it is **assigned to the
...
>   | `josh+qa-client-linked@` | contact _QA ClientA_  | **the LINKED client** — must read the row. Proves the grant exists. |
>
...
solely so cross-company isolation can be _proved_ rather than asserted from code:
...
runs in _both_ directions and A had no payments of its own to hide.

**D-57 punch fixtures [S113].** Company A's fixture project also carries a punch list
_QA — D-57 fixtures_ with three items, seeded permanently so the D-57/D-58 proof is reproducible by
...
- **Client portal messaging.** ~~Real-time chat or async email-style? Real-time is more work. Tied to the
  ...
  R11/R13].** The client adds _photos, notes and questions_; Owner/Admin/PM respond directly; it is **a
...
interview settled it as **R1 — accounts, not magic links.** _Josh's reasoning, as given:_ long-lived
...
| ------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- |
| Is FrameFocus the portal, the company website, or both? | **FrameFocus**, with accounts. A company site may carry a _link_ to the invite (R1's second entry point), never the portal itself.                             | R1          |

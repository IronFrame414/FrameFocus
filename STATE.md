...
Last updated: 2026-07-10 — Session 64 (Module 6 specs complete 6A–6E; 6A Session-64 reversals applied; 6E deferred post-launch; no Module 6 code built; signed-artifact build exists on unmerged branch feat/signed-artifacts)
...
| 5. Project Management         | 🚧 IN PROGRESS | **5A ✅, 5B ✅, 5C ✅, 5D ✅, 5E ✅ — launch scope built, merged to main (PR #1, `27a2d03`), schema live on prod.** `company_members` foundation built (`20260704210000`). Six M5 migrations on prod. Identity convention: all assignment via `company_members(id)` / `member_id`. Specs: `docs/specs/5A-spec.md` through `5E`, architecture in `docs/specs/module5-architecture.md`. Functional tests vs. throwaway: #1 conversion PASS, #2 RLS PASS, #3 punch gate PASS, #4 CO create/send NOT RUN. Post-launch (spec-only, NOT built): 5F Templates, 5G Closeout & Warranty, 5I Subcontractor Portal. 5H Activity Log unspecced.<br><br>**Unmerged branch `feat/signed-artifacts` (signed CO artifacts, Module 5D):**<br>– Branch `feat/signed-artifacts` is pushed and UNMERGED.<br>– It carries migration `20260710120000_signed_artifacts.sql`, which has NOT been applied to any database — not the throwaway and not production.<br>– The migration adds five contractor-signature columns and three reminder columns to `change_orders`, `contractor_signature_path` to `companies`, two FK columns to `email_logs`, and swaps the `email_logs` `email_type` CHECK to eleven values. That CHECK swap is the only destructive operation.<br>– The eleven values include `safety_incident` and `material_delivery`, seeded ahead of Module 6 so `email_logs` is altered once, not twice.<br>– The branch also carries the full CO signing, PDF, email, and reminder-cron build across four commits.<br>– 43 TypeScript errors exist on that branch, all of the "column not yet present in `database.ts`" class. They resolve when the migration applies and `npm run db:types` regenerates.<br>– Nothing on that branch has been run end to end. |     |
| 6. Team & Field Operations    | 📝 SPEC COMPLETE | **6A Time Tracking, 6B Daily Logs, 6C Safety Incidents, 6D Material Deliveries, 6E Crew Briefings.** Specs: `docs/specs/6A-spec.md` through `6E-spec.md`. No Module 6 code exists anywhere in repo history — verified by pickaxe across all branches. Traces: 6A PROPOSED (reconstructed against a real Bishop day, Session 64), 6B PROPOSED, 6C PROPOSED, 6D PROPOSED, 6E INVENTED (no field basis). **6E DEFERRED to post-launch** — Josh does not run crew briefings. Build order: 6A first, alone; then 6B, 6C, 6D (6B depends on 6A). Session 64 reversals now in 6A: Owner has a timeclock and attributes segments to projects; Owner hours carry no approval state; breaks may be paid per company setting; only Owner and Admin may edit hours; an edit does not clear approval. Paid-break settings belong to the batched Company Settings pass, not a 6A migration. Deferred to M5: in-app material lists and measurement capture (raised Session 64). |

...
import { isBoundaryHour, zonedDayRangeUtc } from '@framefocus/shared/utils/notify-hours';
 ...
 */
export async function runDailyLogMissing(
  ...
  for (const company of companies ?? []) {
    ...
    const dayRange = zonedDayRangeUtc(logDate, tz);
    ...
    try {
      ...
      const { data: segments } = await admin
        ...
        // ⚠️ THE DAY WINDOW IS COMPUTED, NOT STRING-CONCATENATED. `segment_start`
        // is timestamptz, and an offset-less literal like `${logDate}T00:00:00`
        // is read by Postgres in the SESSION timezone (UTC here) — for a New
        // York company that is a window shifted four hours, missing the evening
        // crew and picking up the previous evening's. Found while writing the
        // live loop test; the string form was in the first cut of this file.
        .gte('segment_start', dayRange.start.toISOString())
        .lt('segment_start', dayRange.end.toISOString());

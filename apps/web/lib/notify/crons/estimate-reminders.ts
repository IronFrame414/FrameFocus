...
export interface ReminderDeps {
  /**
   * THE SENDER, INJECTED. [S123 coverage pass]
   *
   * ===========================================================================
   * WHY AN INJECTED SENDER AND NOT RESEND'S TEST ADDRESSES
   * ===========================================================================
   * Resend does provide reserved addresses — delivered@resend.dev,
   * bounced@resend.dev — and they are the right tool for asking "does our
   * template render and does Resend accept it". They are the WRONG tool here,
   * for two reasons:
   *
   *   1. THEY REPLACE THE RECIPIENT, WHICH IS THE FACT UNDER TEST. What was
   *      untested about this loop is WHO it decides to email and when. You
   *      cannot assert "the reminder went to the client and the exhausted
   *      notice went to Owner+Admin" while every address is rewritten to
   *      delivered@resend.dev.
   *   2. They need a valid API key and a live network call. This environment's
   *      key is not valid — 161 of the email_logs failure rows say "API key is
   *      invalid" and 9 say it is unset — so those tests would fail for a
   *      reason unrelated to the code, and would be network-flaky if fixed.
   *
   * ⚠️ AND TODAY'S APPARENT SAFETY IS ACCIDENTAL, NOT ISOLATION. Nothing leaves
   * this environment right now only because the key is broken. That is a
   * property of the environment, not of the tests, and it would silently start
   * mailing real addresses the day somebody fixes the key. The injection is the
   * isolation.
   *
   * WHAT THIS DOES NOT COVER, STATED PLAINLY: an injected sender proves the loop
   * called it with the right recipients, subject and body. It proves NOTHING
   * about whether Resend accepted the message, whether the template renders, or
   * whether it was delivered. That is a different test and it needs a real key
   * and a reserved address.
   */
  send: (params: SendEmailParams) => Promise<{ messageId: string | null; error: string | null }>;
  /** `NEXT_PUBLIC_APP_URL` in production; supplied so links are assertable. */
  appUrl: string;
}

/** The loop, with the clock and the sender injected. */
export async function runEstimateReminders(
  ...
  if (fetchError) {
    return { checked: 0, remindersSent: 0, expired: 0, errors: [fetchError.message] };
  }
  ...
  return { checked: estimates.length, remindersSent, expired, errors };
}

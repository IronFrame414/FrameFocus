...
const MAX_MS = Number(process.env.S112_CDN_MAX_MS ?? 8 * 3600_000);
const BUCKET = 'project-files';
const EMAIL = `disposable-s112-cdn-probe-${Date.now()}@example.invalid`;
...
async function teardown() {
  if (companyId) {
    ...
    const all = [...Object.values(paths), ...((files ?? []) as { file_path: string }[]).map((f) => f.file_path)].filter(Boolean);
...
beforeAll(async () => {
  if (!RUN) return;
  assertRebuildTest();
  const { data: u, error } = await admin.auth.admin.createUser({ email: EMAIL, password: PASSWORD, email_confirm: true });
  expect(error, error?.message).toBeNull();
  userId = u.user!.id;
  const { data: prof } = await admin.from('profiles').select('id, company_id').eq('user_id', userId).single();
  companyId = (prof as { company_id: string }).company_id;
  await admin.from('companies').update({ name: 'DISPOSABLE S112 CDN PROBE — delete me' }).eq('id', companyId);
  const up = await admin.from('profiles').update({ role: 'crew_member' }).eq('id', (prof as { id: string }).id);
  expect(up.error, up.error?.message).toBeNull();
  const { data: m } = await admin.from('company_members').select('id').eq('profile_id', (prof as { id: string }).id).single();
  memberId = (m as { id: string }).id;
  const { data: c, error: cErr } = await admin
    .from('contacts')
    .insert({ company_id: companyId, first_name: 'Disposable', last_name: 'Probe', contact_type: 'client' })
    .select('id')
    .single();
  expect(cErr, cErr?.message).toBeNull();
  const { data: p, error: pErr } = await admin
    .from('projects')
    .insert({ company_id: companyId, name: 'DISPOSABLE S112 CDN PROBE', contact_id: (c as { id: string }).id, project_type: 'fixed_price', project_number: 'PRJ-DISPOSABLE', project_internal_seq: 1 })
    .select('id')
    .single();
  expect(pErr, pErr?.message).toBeNull();
  projectId = (p as { id: string }).id;
  const a = await admin.from('project_assignments').insert({ company_id: companyId, project_id: projectId, member_id: memberId, role_on_project: 'crew' });
  ...
  for (const k of ['P1', 'P2', 'N1'] as const) {
    ...
    const ins = await admin.from('files').insert({ id, company_id: companyId, project_id: projectId, category: 'photos', file_name: `disposable-${k}.webp`, file_path: path, file_size: bytes.length, mime_type: 'image/webp' });
...
it.skipIf(!RUN)('a revoked token: when do its reads stop?', async () => {
  ...
  const t0 = Date.now();
  console.log(`[Q6 long] REVOKED at ${new Date(t0).toISOString()}; token exp ${new Date(tokenExp * 1000).toISOString()} (t+${Math.round(tokenExp - t0 / 1000)}s)`);
  ...
  while (Date.now() - t0 < MAX_MS) {
    ...
    console.log(`[Q6 long] t+${sec}s (token ${pastExp > 0 ? `expired ${pastExp}s ago` : `valid ${-pastExp}s more`}) P1 ${p1.s}/${p1.cf} P2 ${p2.s}/${p2.cf} N1(never primed) ${n1.s}/${n1.cf}`);
    closedTicks = p1.s !== 200 && p2.s !== 200 ? closedTicks + 1 : 0;
    if (closedTicks >= 3) break;
    await new Promise((r) => setTimeout(r, TICK_MS));
  }
  console.log(`[Q6 long] RESULT (seconds after revocation): ${JSON.stringify(first)}; token lifetime ended at t+${Math.round(tokenExp - t0 / 1000)}s`);
}, MAX_MS + 600_000);

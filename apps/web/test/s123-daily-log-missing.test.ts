...
const source = readFileSync(
  fileURLToPath(new URL('../lib/notify/crons/daily-log-missing.ts', import.meta.url)),
  'utf8'
);
...
describe('the cron wiring', () => {
  it('the ROUTE is secured, and the loop only runs after the gate', () => {
    ...
    expect(routeSource.indexOf('CRON_SECRET')).toBeLessThan(routeSource.indexOf('runDailyLogMissing('));

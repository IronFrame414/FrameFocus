...
export default function ResetPasswordForm({ recovery }: { recovery: boolean }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    // [S110 E1] The server decides: a recovery-link session sets a password
    // outright; any other session must prove the current one. It returns where
    // this role lives (S109: dashboardDeniedRedirect, the helper middleware uses).
    const result = await resetPasswordFromPage({
      newPassword: password,
      confirmPassword: confirm,
      currentPassword: recovery ? undefined : current,
    });
    setLoading(false);
    if (!result.ok) {
    ...
    <main className="flex min-h-screen items-center justify-center bg-gray-50 px-4">
      <div className="w-full max-w-md rounded-xl bg-white p-8 shadow-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {!recovery && (
            <div>
              {/* [S110 E1] No recovery link behind this session — so it must
                  prove the current password, like the Account page (#162). */}

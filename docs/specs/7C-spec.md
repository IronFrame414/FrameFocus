...
**Status:** SPEC — §7.9 approved trace (stands) + S89 gap decisions + Phase 2 answers confirmed.
Not built.
...
S89 gap decisions (this spec's §0); the matching 7A-spec amendments ship in the same pass.
**Phase 1 verification:** claims cited file:line against the live repo this session.
**Protocol:** branch `feature/7a-spec`, uncommitted. No SQL run. Files touched: this spec +
the approved `7A-spec.md` boundary amendments only.
...
job reads **$1,500 actual + $500 committed** — each payment moves its amount to actual **at
payment time**; committed holds only the remaining balance. There is no committed-until-full
...
- A **closed-out row exits every committed Σ** (service + rollup filter `closed_out_at IS NULL`).
  ...
  at build). It drops the remaining balance only — dollars already paid stay actual (S89
  close-out: live split accounting, §2.2).
...
- **Writer gating:** the 7A column-scope trigger (`enforce_expenses_column_scope`) is extended,
  and a BEFORE INSERT twin added: non-Owner/Admin/PM can never set `state='committed'`,
  `cost_category='subcontractor'`, or any 7C column above. PM writes land `status='pending'`
  per the 7A uniform gate (decision 3: owner/admin/PM enter bills; foreman/crew never).
...
CREATE TABLE public.expense_payments (
    ...
    method      text,            -- 'check' | 'ach' | 'card' | 'cash' | free text (PROPOSED: free text v1)
    note        text
);
```
...
- **Multiple rows per expense are legal (partial payments).**
  ...
  **Representation (PROPOSED — the simpler of the two, recommended): derived at read.** The
  ...
  non-parking-lot option. PROPOSED; confirm at build.
- Over-stage: a payment pushing Σ above `amount` **flags** (stored on the payment note +
  surfaced), override allowed (§7.9 `:576`). Never blocks.
...
- No UPDATE/DELETE policies — a recorded payment is a record; corrections are Owner/Admin
  soft-delete + re-entry (PROPOSED).
...
flag) held until work done + lien release received — release surfaces the 5I compliance state,
advisory.
...
cash_out_7C(job)         = Σ expense_payments on the job's 7C rows
...
- `closeoutCommitment(expenseId, reason)` — Owner/Admin, reason required; flags the sub
  "did not finish" (decision 8; sub-record flag placement PROPOSED — confirm `subcontractors`
  vs `company_members` at build, TECH_DEBT #79 baseline note in 5I `:182`).
- `releaseRetainage(subContractId, payment)` — Owner-only.
...
| --- | --- |
| `/dashboard/expenses` | tabs: **Receipts \| Bills & Commitments \| Review queue** |
| `/dashboard/projects/[id]/costs` | **Payables** section (7A Job Cost tab) |
| `/dashboard/projects/[id]/contracts` | payment-schedule setup + stage/payment panel on the sub contract (`contracts-panel.tsx`) |
| `/dashboard/subcontractors/[id]` (sub record) | compliance docs list + upload + expiry chips |
| `/dashboard/schedule` | compliance expiry trigger points as calendar entries (surface REAL — `schedule/page.tsx:2-28`, `getCalendarEvents`; entry mechanism confirmed at build) |
...
| Files pipeline | `files-client.ts:30-100`; `files.expense_id` (7A §2.4) | bill PDFs (`'invoices'`), compliance PDFs; awaiting-paper clears on attach |
...
6. **"Did not finish" flag placement** — sub record vs member row; blocked on the
   `subcontractors` baseline question (5I `:182` / #79-closed baseline). Confirm at build.
...
flip-at-full rule). Not committed —
Josh commits manually. No SQL run. Anything labeled PROPOSED is a build-time decision surface.*

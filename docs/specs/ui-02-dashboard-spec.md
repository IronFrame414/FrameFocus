...
**Amended 2026-07-20, round 2:** ui-01 **moved** `CompanyCalendar` out of the dashboard into `/schedule`, so this task's new schedule card is the dashboard's **only** schedule surface — build it, don't expect an existing one (§S1, §S3). **Financial Visibility Floor (ui-01 §11):** the **Contract Value** KPI and the **$ amounts** in the Awaiting-Signature KPI + attention feed are **hidden for PM/foreman/crew** (§S2, §4). Vendor schedule events chip as **"Sub"** until tech-debt #89 is fixed at the source (§S3).
...
- **§S1 — Existing dashboard.** Locate the current dashboard route/component. This **restyles it in place**; keep existing data wiring and routing. **Amended round 2:** ui-01 has already **moved** the old company-wide `CompanyCalendar` out of the dashboard into `/schedule` — so the dashboard no longer has a schedule view to keep. Build the new "This week — crew schedule" card (§4) fresh against the same `getCalendarEvents` model; it is the dashboard's only schedule surface.
- **§S2 — KPI data sources.** **Amended 2026-07-20 (audit-confirmed):** none of the four aggregates exist company-wide today — all existing project/CO/punch queries are single-project. **BUILD three net-new read-only, RLS-scoped company-wide reads** (service-layer pattern; do not fabricate numbers, do not invent tables): total contract value, COs awaiting signature (count + sum), open punch count. Active-project count can reuse `getProjects({status:'active'}).length` or a light head-count. Definitions:
  ...
  2. **Contract Value** = sum of revised contract (original `contract_value` + signed CO net deltas) across active projects. *Net-new company-wide sum — no aggregate over `projects.contract_value` exists today.* **Financial floor (ui-01 §11):** this entire KPI card is **hidden for PM/foreman/crew** — the row **reflows to a 3-up** of the remaining cards (no placeholder slot, no substitute metric — deterministic reflow rule, ui-01 §11).
  3. **Awaiting Signature** = **count** of COs where **`status = 'sent'`** (the real "awaiting client signature" state; there is no `awaiting_signature` value). *Net-new — no cross-project CO query exists today.* **Financial floor (ui-01 §11):** the **count is visible to all**, but the **dollar sum** in the caption is **hidden for PM/foreman/crew** (show count only, no "$… in change orders" caption for them).
...
2. **Contract Value** — `$221.7k` style; caption `#6b7280` ("across active jobs"). **Hidden for PM/foreman/crew** (ui-01 §11) — see §S2.2.
3. **Awaiting Signature** — number; caption `#d97706` weight 600 ("$335k in change orders"). **For PM/foreman/crew: show the count, drop the `$` caption** (ui-01 §11) — see §S2.3.
...
- The **+ New Project** button routes per §S5. *(Amended round 2: only one header button now.)*

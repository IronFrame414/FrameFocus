...
| **A20** | **Floor-document amendment**                     | `money-representation.md` and `7d1-spec.md` §12a still state the **overturned** S97 carve-out (_"a PM sees the amounts on an invoice they can reach"_). §7.1 obliged the amendment; the fix commit did not make it. **The Floor documents currently disagree with the shipped Floor.** Parked with the invoice-floor work.                     |

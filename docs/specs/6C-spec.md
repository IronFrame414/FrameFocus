...
> 🚧 **NEEDS INTERVIEW — Josh must narrate a real Bishop safety incident with real details before this trace is authoritative.** A real incident seeded it, but names, project, and date are reconstructed; this pass did **not** promote any of it to fact. The values below remain the pre-existing PROPOSED draft, unchanged.

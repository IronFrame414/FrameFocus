...
invoice. **[S96 extension]** The mirror also holds: every client **credit** ties to a credit document
...
> **[OPEN — JOSH] Percentage-of-source has no worked example.** It is offered here and tested by
> acceptance #1, but every real trace in §15 bills an **edited fixed amount**. Per §2a a calculated
> variant needs a worked example from real input. Either supply one real draw billed as a percentage,
> or rule the method out of v1. **Until then it is specced but unproven.**
...
### §4a — Negative change orders (client removes scope) — **[S96]**

Architecture §7.11 defines the behavior and architecture §7.2 assigns _"negative-CO credits"_ to 7E,
but neither spec carried it. **Ruling: the work splits.** (Recorded as an architecture amendment in
**§A.5** — **[S97]**, which the prior revision omitted.)
...
> **[OPEN — JOSH] §4a has no worked example.** It is net-new, client-facing, and exports to QB.
> Needs one real deductive CO to become an acceptance trace (§15).
...
## §8 — Derivation, override, and disposition — **[S96]**

Applies to every derived invoice (§6 cost-plus and §7 T&M). One mechanic for both.

**The system derives; the user may override.** FrameFocus computes the amount from approved costs and
hours at the rates in force, and presents it as a **proposal**. The user may change it per invoice,
and **the override is stored as an override** — not as a silent replacement of the derivation. Both
numbers survive on the line.

**A downward override prompts for disposition.** Whenever the billed amount is _below_ the derived
amount, the invoice asks what happens to the shortfall:

| Disposition   | Meaning                                                                                                          |
| ------------- | ---------------------------------------------------------------------------------------------------------------- |
| **Write off** | The underlying costs/hours are marked billed. The difference is a permanent margin write-down. It never returns. |
| **Hold back** | The shortfall stays **unbilled** and remains available on a later invoice. §6.2's cost picker must surface it.   |

Both must be built — this is the superset, not a shortcut. The prompt fires **only** on a downward
override, never on every send.

**Upward overrides.** Permitted, with **no disposition prompt** — there is no shortfall to dispose of.
The derived and billed figures still both persist, so the difference stays visible.

> **[OPEN — JOSH] Neither real trace exercised an override.** §15-B and §15-C both billed the derived
> figure exactly. This is a large build resting on an unconfirmed behaviour. **Confirm that downward
> overrides happen in practice, and supply one real instance**, or the write-off / hold-back pair
> should be reconsidered for v1.
...
**Drafts re-derive; overrides survive.** A draft **re-derives** when a rate or an underlying cost row
changes before send, and any override **is preserved** — but the user is **notified that the
underlying figure moved**, so an override entered against one derivation is never silently carried
onto a different one. Once sent, nothing re-derives.
...
> **[OPEN — JOSH] How does labor render in layout A?** Labor bills at hours × flat rate, not cost +
> markup, so it does not fit the Subtotal / Markup / Total shape. Neither real trace carried crew
> labor. Needs one cost-plus or T&M invoice **with crew hours on it** to pin the layout.
...
- The **pay link** is QuickBooks-hosted (7G Model A); FrameFocus shows a clear notice before
  redirecting the client. **[S96]** This is why the Pre-Module 9 gate no longer blocks electronic pay
  — see §O.
...
INPUT   Cost-plus ESTIMATE-CONTRACT — the original contract, not a CO.
        ...
        User opens the cost picker and selects five approved costs (§6.2):
          subcontractor #1    $1,200.00
          subcontractor #2    $1,800.00
          subcontractor #3      $275.00
          lumber                $958.48
          plumbing fixtures     $625.20
...
STORE   Invoice lines carry cost basis, CATEGORY, THE RATE ROW APPLIED, derived
        $5,830.42, billed $5,830.42, disposition null. (§8)
...
> **[OPEN — JOSH]** the client-facing description for each line (what replaces "subcontractor #1"),
> and the five incurred dates. Neither changes the math here, because one rate ran the whole job —
> but without dates the trace cannot demonstrate rate-in-force selection.
...
STORE   Lines carry hours/cost basis, CATEGORY, THE RATE ROW APPLIED, derived
        $4,612.08, billed $4,612.08, disposition null. (§8)
...
### Traces still owed — **[OPEN — JOSH]**

| Missing                                            | Why it matters                                                   |
| -------------------------------------------------- | ---------------------------------------------------------------- |
| **Percentage-of-source draw**                      | Offered in §2, tested by acceptance #1, demonstrated nowhere     |
| **Negative-CO credit document**                    | §4a is net-new, client-facing, and exports to QB as a CreditMemo |
| **A derived invoice with an override**             | §8's write-off / hold-back pair is unexercised                   |
| **A cost-plus or T&M invoice carrying crew labor** | Pins §11 layout A's treatment of labor                           |
...
11. **[S96]** A downward override **prompts** for write-off vs. hold-back; a held-back shortfall
    reappears in the next invoice's cost picker; a written-off one never does.
12. **[S96]** Both the **derived** and the **billed** amount survive on the line, and 7G/7H consume
    **billed**.
...
18. An invoice can be delivered by email (pay link + PDF) or printed (skip email); both save it to
    the project.
19. **[S97]** All three presentation levels are selectable per invoice, and **full detail renders as
    layout A** — actual cost per line, then a separate subtotal and markup line — with **unburdened**
    cost.
20. No income exists in the system that is not tied to an invoice, **[S96]** and no client credit
...
**A.5 — Negative-CO credits split between 7D and 7E — [S97, previously unrecorded].** Architecture
§7.2 assigns _"negative-CO credits"_ to **7E** and §7.11 traces the behaviour there. §4a rules that
**7D issues the credit document** and **7E applies it**. This divergence was ruled but never recorded
as an amendment. Amend §7.2 and §7.11, or reverse the ruling.
...
1. **Signed-artifact / change-order tables** — 7D converts signed COs into invoices and, per §4a,
   issues a **credit document** off a signed negative CO. _(Merged and live: `change_orders` —
...
| #   | Item                                                                            | Section  |
| --- | ------------------------------------------------------------------------------- | -------- |
| 1   | A real draw billed as a **percentage of source** — or rule the method out of v1 | §2, §15  |
| 2   | A real **negative CO** to trace the credit document                             | §4a, §15 |
| 3   | Confirmation that **downward overrides** happen, with one real instance         | §8       |
| 4   | One invoice **carrying crew labor**, to pin layout A's labor treatment          | §11      |
| 5   | Client-facing **line descriptions** and **incurred dates** for trace B          | §15-B    |

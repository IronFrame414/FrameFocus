...
| ------ | --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------- |
| **7A** | Job Cost Ledger                   | The spine. Cost rows: labor / material / subcontractor / other. Cost per row ~~+ sell~~ [S89 reversal, applied to this row S91 — no sell column shipped]. Committed vs. actual (cash basis). Burden on labor rows. ~~Verification lifecycle~~ Approval lifecycle (`pending\|approved\|rejected`) [S91]. Receipt attachments. Rolls up to job cost, compared against the M5 budget. | **YES** — traced §7.8 (INVENTED)         |
| **7B** | Contract Value & CO Write-Through | Owns debt #1. Original contract value + signed change-order deltas — **bidirectional** (a negative CO lowers the total); material-selection COs ride the same path. [S91: shipped as DERIVATION at read (`contract-value.ts`; #80 closed by derivation, S90) — `projects.contract_value` is never mutated. Sub-module name retained for history; the mechanism is not a write-through.] | No — a rule, not a workflow              |
...
- **7G exports** from 7A/7C/7D — it is downstream of anything that produces exportable data.
  **[S91 footnote — diagram not redrawn:** per 7E §A.1 / 7G §7G.5 (both confirmed), 7G is ALSO
  **upstream of 7E's electronic-payment path** — payments process *through* QuickBooks and flow
  back in; the non-QB half of 7E builds first, the electronic half stubs until 7G. Additionally,
  the 7B arrow is a **derivation at read** (`contract-value.ts`), not a write-through — see the
  amended §7.2 row.]

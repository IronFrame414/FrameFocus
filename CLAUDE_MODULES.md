...
>
> **BUILT — Session 64 (branch `feat/module-6a`).** 6A is built, not spec-only: migration `20260710130000_module6_6a_time_tracking.sql` (**two tables** — `time_clock_sessions` + `time_segments`), service layer, and pure derivation. Migration **written, not yet applied to any database.** **Session-64 reversals:** (a) **Owner has a timeclock** — Owner labor is job cost; (b) **Owner hours carry no approval state** (`status` nullable; never pending, never approved); (c) **breaks may be paid** per a company setting (paid-break-minutes-per-day cap), which counts toward paid hours and the OT threshold but never lands on a job — forcing the **paid-hours ≠ worked-hours** split; (d) **only Owner/Admin may edit hours** and an edit does **not** clear approval. **Deferred to the batched Company Settings pass (not a 6A migration):** the four settings columns — OT threshold, paid-break toggle + cap, GPS capture/enforce, admin-timeclock; 6A derives hours from pure functions taking these as parameters (defaults: unpaid breaks, 40 h/wk).

...
class _ProjectsScreenState extends State<ProjectsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error loading projects: $e'), backgroundColor: AppColors.error));
    }
  ...
  void _showCreateDialog() {
    ...
    showDialog(context: context, builder: (ctx) => StatefulBuilder(builder: (ctx, setS) => AlertDialog(
      ...
      content: SizedBox(width: 520, child: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
        TextField(controller: nameC, decoration: const InputDecoration(labelText: 'Project Name *', hintText: 'e.g. Riverside Office Complex')),
        const SizedBox(height: 12),
        Row(children: [
          Expanded(child: DropdownButtonFormField<String>(
            value: projectType, decoration: const InputDecoration(labelText: 'Project Type'),
            items: ['Residential', 'Commercial', 'Industrial', 'Multi-Family', 'Renovation', 'Other'].map((t) => DropdownMenuItem(value: t, child: Text(t))).toList(),
            onChanged: (v) => setS(() => projectType = v!),
          )),
          const SizedBox(width: 12),
          Expanded(child: DropdownButtonFormField<String>(
            value: status, decoration: const InputDecoration(labelText: 'Status'),
            items: ['planning', 'active', 'on_hold'].map((s) => DropdownMenuItem(value: s, child: Text(s.replaceAll('_', ' ')))).toList(),
            onChanged: (v) => setS(() => status = v!),
          )),
        ]),
        const SizedBox(height: 12),
        TextField(controller: descC, decoration: const InputDecoration(labelText: 'Description', hintText: 'Describe the scope of work...'), maxLines: 3),
        const SizedBox(height: 12),
        TextField(controller: budgetC, decoration: const InputDecoration(labelText: 'Budget', prefixText: '\$ ', hintText: '0.00'), keyboardType: TextInputType.number),
        const SizedBox(height: 16),
        const Align(alignment: Alignment.centerLeft, child: Text('Location', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.textSecondary))),
        const SizedBox(height: 8),
        TextField(controller: addressC, decoration: const InputDecoration(labelText: 'Street Address', hintText: '1234 Main St')),
        const SizedBox(height: 12),
        Row(children: [
        ...
        const SizedBox(height: 16),
        const Align(alignment: Alignment.centerLeft, child: Text('Schedule', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.textSecondary))),
        const SizedBox(height: 8),
        Row(children: [
        ...
        ElevatedButton(
          onPressed: saving ? null : () async {
            if (nameC.text.isEmpty) {
              ScaffoldMessenger.of(ctx).showSnackBar(const SnackBar(content: Text('Project name is required')));
              return;
            }
            setS(() => saving = true);
            try {
              ...
              if (ctx.mounted) {
                Navigator.pop(ctx);
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Project "${nameC.text}" created!'), backgroundColor: AppColors.success));
              }
              _load();
            } catch (e) {
              setS(() => saving = false);
              if (ctx.mounted) ScaffoldMessenger.of(ctx).showSnackBar(SnackBar(content: Text('Error: $e'), backgroundColor: AppColors.error));
            }
          },
          child: saving ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : const Text('Create Project'),
        ),
      ],
    )));
  }
  ...
  Widget build(BuildContext context) => RefreshIndicator(
    ...
    child: ListView(padding: const EdgeInsets.all(24), children: [
      ...
      else if (_filtered.isEmpty) EmptyState(icon: Icons.folder_open, title: 'No projects found', subtitle: 'Create your first project to get started', buttonLabel: '+ New Project', onAction: _showCreateDialog)

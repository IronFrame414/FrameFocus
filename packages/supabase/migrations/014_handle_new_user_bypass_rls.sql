-- Migration 014: Make handle_new_user() bypass RLS on invitations lookup
-- Bug: RLS policy on invitations requires get_my_company_id() and get_my_role(),
-- which both return NULL during the signup trigger (profile doesn't exist yet).
-- Fix: Explicitly set row_security = off inside the function.
